...
- 裸 `/kkgifzip` 与 `/kkgifzip1` 同为第 1 档；数字越大越糊。
- 支持：一个视频，或一个 GIF。
- 不支持：静态图片（jpg/png 等），会直接提示。
- 视频最长 16 秒；输出不含声音。
- 纯本地 FFmpeg，不调用模型。
- **糊感来源**（不是靠降帧）：更小边长、更少色、**crush 像素块**（缩小再 neighbor 放大）、轻模糊、高档关闭抖动。
- 各档帧率固定约 **10 FPS**，避免卡顿。

档位预设：

| 档 | 边长 | FPS | 色数 | crush | 模糊 | 抖动 |
|----|------|-----|------|-------|------|------|
| 1 | 280 | 10 | 96 | ×1.6 | 0.35 | bayer |
| 2 | 220 | 10 | 64 | ×2.0 | 0.55 | bayer |
| 3 | 180 | 10 | 48 | ×2.5 | 0.75 | none |
| 4 | 140 | 10 | 32 | ×3.2 | 0.95 | none |
| 5 | 100 | 10 | 24 | ×4.0 | 1.2 | none |

输出仍受 `video_gif_max_bytes` 限制；过大时会在档内再砍分辨率/色数并加强 crush，**不降 fps**。
